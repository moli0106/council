    ...
    public function index(){
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            if($registration_for == 1){
                ...
                $config[] = array('field' => 'exam_type_id','label' => 'Exam Type / Application Name','rules' => 'trim|required');
                $config[] = array('field' => 'poly_course_id','label' => 'Course Name','rules' => 'trim|required');

                $this->form_validation->set_rules('fullmark', '<b>Full Marks</b>', 'trim|required|numeric');
                $this->form_validation->set_rules('marks_obtain', '<b>marks_obtain</b>', 'trim|required|numeric');
                ...
                $this->form_validation->set_rules('passing_year', '<b>Passing Year</b>', 'trim|required|exact_length[4]|numeric');
    ...
    public function getVtcName($vtcCode = NULL)
    {
        if (!$this->input->is_ajax_request()) {
            ...
            if($registration_for == 1){
                ...
                $institute_details = $this->student_reg_model->geteInstituteDetails($vtcCode);

                if (!empty($institute_details)) {

                   
                    $response_data = array(
                        'vtc_name' => $institute_details[0]['institute_name'],
                        'udise_code' => $institute_details[0]['aishe_code']
                    );

                    // echo json_encode($institute_details[0]['vtc_name']);
                    echo json_encode($response_data);
