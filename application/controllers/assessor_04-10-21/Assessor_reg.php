...
class Assessor_reg extends NIC_Controller {
    ...
    public function index(){
        ...
        $this->form_validation->set_rules('pan', '<b>PAN</b>', 'trim|required|regex_match[/^[A-Z]{3}[ABCFGHLJPT][A-Z][0-9]{4}[A-Z]$/]|[council_assessor_registration_details.pan]',
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if($insert_id = $this->assessor_reg_model->insert_assessor_draft($assessor_basic)) {
                ...
                //send otp
                //$this->load->library('sms');
                $sms_message ="Your mobile verification code for registration as Assessor/ Expert / Trainer of Trainers for WBSCTVESD under TETSD is ". $this->session->sms_otp;
				$template_id=1107161201120409862;
                $this->sms->sendotp($this->input->post("mobile_no"),$sms_message,$template_id);
				...
				if(count($data["assessor"])){
					...
					if ($this->form_validation->run() == FALSE) {
						...
						if($this->input->post("submit") == "resend_otp"){
							...
							$template_id=1107161201120409862;
							$this->sms->sendotp($data["assessor"][0]['mobile_no'],$sms_message,$template_id);
							...
							if($this->input->post("mobile_otp") == $this->session->sms_otp){
								...
								if($new_insert_id = $this->assessor_reg_model->assessor_migrate($assessor_draft,$insert_id_hash)){
									...
									if($this->assessor_reg_model->create_login($login_array)){
										...
										$template_id=1107161201130031686;
