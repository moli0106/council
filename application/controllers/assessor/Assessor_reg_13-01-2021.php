...
class Assessor_reg extends NIC_Controller {
    ...
    public function index(){
        ...
        $this->form_validation->set_rules('mobile_no', '<b>Mobile No.</b>', 'trim|required|exact_length[10]|integer',
        ...
        $this->form_validation->set_rules('email_id', '<b>email ID</b>', 'trim|required|max_length[50]|valid_email');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if($insert_id = $this->assessor_reg_model->insert_assessor_draft($assessor_basic)) {
                ...
                //send otp
                //$this->load->library('sms');
                $sms_message ="Your OTP is ". $this->session->sms_otp ." from WBSCTVE & SD";
                //$this->sms->sendotp($this->input->post("mobile_no"),$sms_message);
            ...
            if(count($data["assessor"])){
                ...
                if ($this->form_validation->run() == FALSE) {
                    ...
                    if($this->input->post("submit") == "resend_otp"){
                        ...
                        //generate otp
                        $this->load->helper('string');
                        echo $this->session->sms_otp = random_string('numeric', 6);
                        $this->session->otp_send_time = (int)time();
                        $this->session->mobile_no = $data["assessor"][0]['mobile_no'];
                        $sms_message ="Your OTP is ". $this->session->sms_otp ." from WBSCTVE & SD";
                        //$this->sms->sendotp($data["assessor"][0]['mobile_no'],$sms_message);
                        ...
                        if($this->input->post("mobile_otp") == $this->session->sms_otp){
                            ...
                            if($new_insert_id = $this->assessor_reg_model->assessor_migrate($assessor_draft,$insert_id_hash)){
                                ...
                                $ass_id = $this->gen_assessor_login_id($new_insert_id);
