...
class Student_reg extends NIC_Controller {
	...
	public function index(){
        ...
        $date_convert = strtotime('2023-11-16');
		...
		if ($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			if($registration_for == 1){
                ...
                $config[]= array('field' => 'aadhar_no','label' => 'Aadhar Number','rules' => 'trim|required|exact_length[12]');
           ...
           // echo "<pre>";print_r($config);die;
			...
			if ($this->form_validation->run() == FALSE) {
                ...
                if ($registration_for == 2){
                    //echo "hii";exit;
                    //check entrance exam for application
                     $entrance = $this->student_reg_model->check_enrance($this->input->post('exam_type_id'));

                    if($entrance == 1){

                        $this->instituteStdRegistration($_POST,$data['academic_year']);
                    }elseif ($entrance == 0) {
                        $this->nonEntranceStdRegistration($_POST,$data['academic_year']); 
                    }
                }
                if($registration_for == 1){
                    if($this->input->post('caste_id') != 1){
    ...
    public function nonEntranceStdRegistration($post_data, $academic_year){
        ...
        $check_data = $this->student_reg_model->checkStudentExist($post_data['aadhar_no']);
        ...
        if(empty($check_data)){
            ...
            $insert_data = array(
                ...
                'institute_id_fk'       => $vtc_details['vtc_id_pk'],
                'spotcouncil_student_details_id_fk' => null,
                //'institute_reg_id_fk'   => $vtc_details['vtc_details_id_pk'],
            ...
            $student_details_id_pk = $this->student_reg_model->insertStdData('council_institute_student_details',$insert_data);
            if ($student_details_id_pk) {
                ...
                $non_entrance = 1;
                redirect('vtc/student_reg/mobile_verify/' . md5($student_details_id_pk).'/'.$non_entrance);
            } else {
        ...
        }elseif (!empty($check_data) && $check_data['final_mobile_no_verify_status'] == 1) {
            redirect('/admin/student_login');
        }elseif (!empty($check_data) && ($check_data['final_mobile_no_verify_status'] == 0 || $check_data['final_mobile_no_verify_status'] == NULL)) {
            
            $non_entrance = 1;
            $updateArray = array(
                'final_mobile_otp'          => $mobile_otp
            );
            $status = $this->student_reg_model->updateStdDetails($check_data['institute_student_details_id_pk'], $updateArray,$non_entrance);
            if ($status) {
                ...
                $this->session->set_flashdata('alert_msg', 'Please Verify Mobile No.');
                $non_entrance = 1;
                redirect('vtc/student_reg/mobile_verify/' . md5($check_data['institute_student_details_id_pk']).'/'.$non_entrance);
            } else {
    ...
    public function mobile_verify($id_hash = NULL , $non_entrance =null)
    {
        if ($id_hash) {
            $data['non_entrance'] = $non_entrance;

            if($non_entrance == 1){
                //echo "if";exit;
                $stdDetails  = $this->student_reg_model->getNonEntranceStdDetailsByIdHash($id_hash);
                $student_details_id_pk = $stdDetails['institute_student_details_id_pk'];
            }else{
                //echo "else";exit;
                    ...
                    if ($this->input->server('REQUEST_METHOD') == 'POST') {
                        ...
                        if ($this->form_validation->run() != FALSE) {
                            ...
                            if ($otp == $data['otp']) {
                                ...
                                $status = $this->student_reg_model->updateStdDetails($student_details_id_pk, $updateArray,$non_entrance);
