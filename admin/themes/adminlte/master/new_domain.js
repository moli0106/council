$(function () {
    ...
    $(document).on('click','.action_buttons a.view_course', function (e) {
        ...
        $.ajax({
            type: "GET",
            url: "master/new_domain/view_course_details/" + course_id_hash,
            beforeSend: function(){
    ...
    $(document).on('click','.action_buttons a.delete_domain', function (e) {
        //e.preventDefault();
        var course_id_hash = $(this).attr("alt");
        $("#myModal .modal-title").html("Delete domain");
        $("#myModal .modal-body").html("Loading...");

        $.ajax({
            type: "GET",
            url: "master/new_domain/delete_domain_details/"+course_id_hash,
            beforeSend: function(){
    ...
    $(document).on("click",".domain_delete_submit", function () {
        ...
        $.ajax({
            type: "GET",
            url: "master/new_domain/delete_domain_details/"+course_id_hash+"/delete",
            success: function (response) {
                $("#myModal .modal-body").html(response);
               setTimeout(function(){ window.location.href="master/new_domain"; }, 2000);
