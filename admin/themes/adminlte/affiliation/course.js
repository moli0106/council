$(document).ready(function() {
    ...
    if(class_name !='' && class_name!=null && class_name!=undefined){
        getsubjectCategoryByClassName(class_name, vtcCourseId);
    }

    $(document).on('change', '#class_name', function() {

        var class_name_id = $(this).val();
        getsubjectCategoryByClassName(class_name_id, vtcCourseId);

    });

    function getsubjectCategoryByClassName(class_name_id, vtcCourseId){

        $.ajax({
            url: "affiliation/subject/getSubjectCategoryByClassName",
            type: 'GET',
            dataType: "json",
            data: { class_name_id:class_name_id, vtcCourseId:vtcCourseId },
        })
        .done(function(res) {
            $('#category_id').html(res);
            Swal.close()
