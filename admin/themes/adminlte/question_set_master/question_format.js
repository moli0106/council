$(document).ready(function () {
    ...
    $(document).on('change', '#question_code_id', function () {
        var question_code_id = $(this).val();
        ...
        Swal.fire({
            ...
            didOpen: () => {
                Swal.showLoading();
                $.ajax({
                    url: "question_set_master/question_format/getQuestionTypeMarks/",
                    dataType: "json",
                    data: { question_code_id: question_code_id },
                })
                    .done(function (res) {
    ...
    $(document).on('change', '#sam_year_id, #discipline_id', function () {
        var course_id = $("#course_id").val();
        var sam_year_id = $("#sam_year_id").val();
        var discipline_id = $("#discipline_id").val();

        if ((sam_year_id != '') && (discipline_id != '')) {
            Swal.fire({
                title: 'Please wait a moment!',
                html: 'We are collecting data...',
                allowEscapeKey: false,
                ...
                didOpen: () => {
                    Swal.showLoading();
                    $.ajax({
                        url: "question_set_master/question_format/getQuestionCode/",
                        dataType: "json",
                        data: { course_id: course_id, sam_year_id: sam_year_id, discipline_id: discipline_id },
                    })
                        .done(function (res) {
                            $("#question_code_id").html(res);
                            Swal.close();
                        })
                        .fail(function (res) {
                            Swal.fire('Warning!', 'Oops! No data found....', 'warning');
                            $("#question_code_id").html('<option value="">Select Question Code</option>');
                        });
                }
            });
        }
    ...
    $(document).on('change', '#course_id', function () {
        ...
        Swal.fire({
            ...
            didOpen: () => {
                Swal.showLoading();
                $.ajax({
                    url: "question_set_master/question_format/getSemesterAndDiscipline/",
                    dataType: "json",
    ...
    $(document).on('click', '.assignQuestion', function (e) {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "question_set_master/question_format/add_question/" + id,
                    dataType: "json",
                })
                    .done(function (res) {
                        console.log(res);
