        ...
        <div class="box box-info">
            <div class="box-header with-border">
                ...
                <?php if($this->session->userdata('stake_id_fk')==18){?>
					...
					<a href="<?php echo base_url('admin/question_set_master/upload_all_hs_question/download_report') ?>" class="btn btn-success btn-sm">
                        <i class="fa fa-download" aria-hidden="true"></i>&nbsp;&nbsp;&nbsp;Download MIS 
                    </a>
                <?php }?>    
                        ...
                        <?php
                            ...
                            if(count($questionList))
                            {
                                ...
                                foreach ($questionList as $key => $question) { 
									...
									$open_time = date('H:i:s', (strtotime($question['start_time'])));
                                    $curent_time=date("H:i:s", time()); 
                                    $previous_date = date("Y-m-d",strtotime('-1 day',strtotime($question['exam_date']))); 
                                    $exam_date = date("Y-m-d",strtotime($question['exam_date'])); 

                                    //if( ($this->session->userdata('stake_id_fk') == 18) || ($this->session->userdata('stake_id_fk')== 23) || ( ($this->session->userdata('stake_id_fk') == 15) && ((date('Y-m-d') == $previous_date) || ((date('Y-m-d') == $exam_date))) ) ){
										
									if( ($this->session->userdata('stake_id_fk') == 18) || ($this->session->userdata('stake_id_fk')== 23) || ( ($this->session->userdata('stake_id_fk') == 15) && ((date('Y-m-d') == $previous_date && ($open_time <= $curent_time)) || ((date('Y-m-d') == $exam_date)) ) ) ){
                                    ?>
