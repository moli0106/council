...
class Assessor_list_model extends CI_Model {
   ...
   public function assessor_details($assessor_id_hash = NULL){
    ...
    public function get_jobroles($assessor_id_hash = NULL){
        $query = $this->db->select("
    ...
    public function get_certificates($assessor_id_hash = NULL){
    ...
    public function get_work_exp($assessor_id_hash = NULL){
    ...
    public function get_assessor_expert($assessor_id_hash = NULL){
        $query = $this->db->select("
    ...
    public function ssc_wbsctvesd_status_update($id_hash, $array)
    {
	 ...
	 public function assessor_details_all($assessor_id_hash = NULL){
        $query = $this->db->select("*")
                ->from("council_assessor_registration_details as assessor")
                ->where(
                    array(
    ...
    public function assessor_jobrole_details_all($assessor_id_hash = NULL){
        $query = $this->db->select("*")
                ->from("council_assessor_registration_jobrole_sector_map")
                ->where(
                    array(
        ...
        public function assessor_assessor_expert_details_all($assessor_id_hash = NULL){
            $query = $this->db->select("*")
                    ->from("council_assessor_registration_assessor_expert_map")
                    ->where(
                        array(
        ...
        public function assessor_assessor_certified_details_all($assessor_id_hash = NULL){
            $query = $this->db->select("*")
                    ->from("council_assessor_registration_certified_map")
                    ->where(
                        array(
        ...
        public function assessor_assessor_work_experience_details_all($assessor_id_hash = NULL){
            $query = $this->db->select("*")
                    ->from("council_assessor_work_experience_map")
                    ->where(
                        array(
		...
		public function assessor_assessor_working_map_details_all($assessor_id_hash = NULL){
                $query = $this->db->select("*")
                        ->from("council_assessor_working_map")
                        ->where(
                            array(
                                "active_status" => 1,
                                "MD5(CAST(assessor_id_fk as character varying)) =" => $assessor_id_hash
                            )
                        )
                        ->get();
                    return $query->result_array();
    ...
    public function insert_archive_assessor_data($archive_insert_array = NULL){
        return $this->db->insert('council_assessor_registration_details_archive',$archive_insert_array);
    }

    public function insert_archive_assessor_jobrole_data($arr = NULL){
        $this->db->insert_batch('council_assessor_registration_jobrole_sector_map_archive',$arr['jobrole_final_arr']);
        $this->db->affected_rows();
    }

    public function insert_archive_assessor_expert_data($assessor_expert_final_arr = NULL){
        $this->db->insert_batch('council_assessor_registration_assessor_expert_map_archive',$assessor_expert_final_arr);
        $this->db->affected_rows();
    }

    public function insert_archive_assessor_certified_data($assessor_certified_final_arr = NULL){
        $this->db->insert_batch('council_assessor_registration_certified_map_archive',$assessor_certified_final_arr);
        $this->db->affected_rows();
    }

    public function insert_archive_assessor_work_experience_data($assessor_work_experience_final_arr = NULL){
        $this->db->insert_batch('council_assessor_work_experience_map_archive',$assessor_work_experience_final_arr);
        $this->db->affected_rows();
    }
	public function insert_archive_assessor_working_map_data($assessor_working_map_final_arr = NULL){
        $this->db->insert_batch('council_assessor_working_map_archive',$assessor_working_map_final_arr);
        $this->db->affected_rows();
    }

    public function update_job_map_status($array,$id_hash)
    {
        $this->db->where('md5(cast(assessor_registration_details_fk as character varying)) =', $id_hash);
        return $this->db->update('council_assessor_registration_jobrole_sector_map', $array);
    }
	...
	//Added by Waseem on 09-07-2021
    public function get_assessor_document($application_no = NULL, $assessor_id_hash = NULL){
		$query = $this->db->select("doc.council_assessor_registration_document_map_id_pk,quali.qualification,doc.institute_name,doc.certificate_no")
			...
			->where(
				array(
                "MD5(CAST(doc.assessor_registration_details_fk as character varying)) =" => $assessor_id_hash,
