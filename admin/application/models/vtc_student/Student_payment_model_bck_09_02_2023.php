    ...
    public function getStdCountByGroup($vtc_id_fk = NULL , $academic_year = NULL){
        ...
        if(!empty($query)){
            ...
            foreach ($query as $key => $value) {
                ...
                // $this->db->where('student.approve_reject_status', 1);
                // $student = $this->db->get()->result_array();

                // $student = $this->db->select('*')->from('council_vtc_student_master')->where('institute_id_fk' , $vtc_id_fk)
                //             ->where('year_of_registration' , $academic_year)->where('course_id_fk',$group_id)->where('active_status', 1)
                //             ->get()->result_array();
                ...
                $this->db->where('A.institute_id_fk',(int)$vtc_id_fk);
               ...
               //echo $this->db->last_query();exit;
                ...
                ->where('request_std.vtc_id_fk',(int)$vtc_id_fk)
