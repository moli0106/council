...
class New_domain_model extends CI_Model {
    ...
    public function get_domain_count(){
        $query = $this->db->select("count(domain_id_pk)")
            ->from("council_domain_master")
            ->get();
        return $query->result_array();
    }
    public function get_domain($limit = NULL, $offset = NULL){
    ...
    public function get_single_domain($domain_id_hash = NULL){
        $query = $this->db->select("*")
            ->from("council_domain_master")

            ->where(
                array(
                    "MD5(CAST(domain_id_pk AS character varying)) =" => $domain_id_hash ,
                    "active_status" => 1
                )
            )
            ->order_by("domain_id_pk","DESC")
            ->get();
        return $query->result_array();
    }
    public function delete_domain($domain_id_hash = NULL){
        $this->db->where('MD5(CAST(domain_id_pk AS character varying)) =', $domain_id_hash);
        return $this->db->delete('council_domain_master');
    }
