...
class Vtc_model extends CI_Model
{
    ...
    public function getVtcListCount($selected_year)
    {
    ...
    public function getVtcList($limit = NULL, $offset = NULL, $orderColumn, $orderType, $selected_year)
    {
        $this->db->select('cavd.*, cavm.*')
            ...
            ->where('cavd.academic_year', $selected_year)
    ...
    public function getVtcListBySearch($limit = NULL, $offset = NULL, $orderColumn, $orderType, $search, $selected_year)
    {
        $this->db->select('cavd.*, cavm.*')
            ->from('council_affiliation_vtc_details AS cavd')
            ->join('council_affiliation_vtc_master AS cavm', 'cavm.vtc_id_pk = cavd.vtc_id_fk', 'left')
    ...
    public function getNewVtcTeacherList($vtc_id = NULL, $academic_year = NULL)
    {
        // $this->db->select('teacher.*, course.group_name, course.group_code, designation.designation_name');
    ...
    public function unblockingVtcData($vtc_id_pk = NULL, $vtc_details_id_pk = NULL)
    {
        $this->db->where('vtc_id_fk', $vtc_id_pk);
        $this->db->where('vtc_details_id_pk', $vtc_details_id_pk);
        $this->db->update('council_affiliation_vtc_details', array('final_submit_status' => 0, 'second_final_submit_status' => 0));
    }
    ...
    public function getVtcListCountBySearch($search, $selected_year)
    {
        $this->db->select('cavd.*, cavm.*')
            ->from('council_affiliation_vtc_details AS cavd')
            ->join('council_affiliation_vtc_master AS cavm', 'cavm.vtc_id_pk = cavd.vtc_id_fk', 'left')
    ...
    // Added By Moli 0n 16-09-2022 for Update Udise Code

    public function updateVTCUdiseCode($vtc_id_pk,$updArray){

        $this->db->where('vtc_id_pk', $vtc_id_pk);

        $this->db->update('council_affiliation_vtc_master', $updArray);
        return $this->db->affected_rows();
    }

    public function getUdiseCodeStatus($vtc_id_pk){

        $this->db->select('*');
        $this->db->from('council_affiliation_vtc_master');
        $this->db->where('vtc_id_pk', $vtc_id_pk);
    ...
    public function getStudentListByInsId($ins_id_fk){
        $this->db->select('institute_student_details_id_pk,spotcouncil_student_details_id_fk,registration_year,exam_type_id_fk,institue_id_fk');
        $this->db->from('council_institute_student_details');
        $this->db->where('institute_id_fk', $ins_id_fk);
        $this->db->where(array(
            'final_save_status' => 1,
            'approve_reject_status'=>1,
            //'council_reject_status' =>0,
            'reg_certificate_status =' => NULL
        ));
        $query = $this->db->get()->result_array();
        if(!empty($query)){
            return $query;
