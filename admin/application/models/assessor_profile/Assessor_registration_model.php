...
class Assessor_registration_model extends CI_Model {
	...
	//new update for final submission
	public function update_assessor_details_app_count($basic_array = NULL,$insert_id_hash = NULL,$assessor_registration_application_no = NULL){
	...
	//25-03-2021 parag
	public function update_assessor_details_for_new_update($basic_array = NULL,$insert_id_hash = NULL,$ssc_wbsctvesd_certified_array_map_table = NULL){

		$this->db->trans_begin();
		$this->db->where('md5(CAST(assessor_registration_details_pk AS character varying)) =', $insert_id_hash);
		$this->db->update('council_assessor_registration_details',$basic_array);


		//$this->db->where('md5(CAST(assessor_registration_details_fk AS character varying)) =', $insert_id_hash);
		//$this->db->update('council_assessor_ssc_wbsctvesd_certified_map',array('active_status' => 0,'assessor_registration_application_no' => $ssc_wbsctvesd_certified_array_map_table['assessor_registration_application_no']));
...
public function get_work_experiences(){
	...
	->group_start() // updated 05-04-2021
			->or_where(
				array(
					"assessor_registration_application_no" => 1,
					"assessor_registration_application_no IS NULL" => NULL
				)
			)
		->group_end()
	->get();
...
public function delete_work_exp_dtls($id_hash = NULL)
	...
	public function get_assessor_jobroles($assessor_registration_details_fk = NULL){
		$query = $this->db->select("map.course_id_fk,map.sector_id_fk,map.job_role_sp_quali,sector.sector_name")
			...
			->group_start() // updated 03-04-2021
				->or_where(
					array(
						"map.assessor_registration_application_no" => 1,
						"map.assessor_registration_application_no IS NULL" => NULL
					)
				)
			->group_end()
			->get();
...
//added by Waseem on 27-02-2021
	...
	public function get_assessor_application_count(){
		$query = $this->db->select("assessor_registration_application_nubmer_id_pk,max(assessor_registration_application_no) as assessor_registration_application_no")
			...
			->limit(1)
			->group_by("assessor_registration_application_nubmer_id_pk")
	...
	public function assessor_document_upload($document_array = NULL,$assessor_array = NULL, $assessor_id = NULL){
		$this->db->trans_begin();

		$this->db->insert('council_assessor_registration_document_map', $document_array);

		$this->db->where('assessor_registration_details_pk', $assessor_id);
		$this->db->update('council_assessor_registration_details', $assessor_array);
