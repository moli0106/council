 ...
 *
 */
class Assessor_registration_add_more_model extends CI_Model {
	...
	public function update_assessor_details($basic_array = NULL,$insert_id_hash = NULL,$application_no=NULL){
		$this->db->where('md5(CAST(assessor_registration_details_fk AS character varying)) =', $insert_id_hash);
		$this->db->where('assessor_registration_application_no', $application_no);
		return $this->db->update('council_assessor_registration_application_nubmer',$basic_array);
	}

	public function get_applied_courses($course_id=NULL,$application_no=NULL,$assessor_id=NULL){
		$query = $this->db->select("count(assessor_registration_jobrole_sector_map_id_pk)")
			->from("council_assessor_registration_jobrole_sector_map")
			->where(
				array(
					"assessor_registration_details_fk" => $assessor_id,
					"course_id_fk"	=> $course_id,
					"assessor_registration_application_no !=" => $application_no,
					"active_status" => 1
				)
			)->get();
		return $query->result_array();
	...
	//25-03-2021 parag
	public function update_assessor_details_for_new_update($count_table_update = NULL, $assessor_registration_application_no = NULL, $ssc_wbsctvesd_certified_array_map_table = NULL, $assessor_id =NULL){
		...
		$this->db->where('assessor_registration_application_no', $assessor_registration_application_no);
		$this->db->update('council_assessor_registration_application_nubmer',$count_table_update);


		// $this->db->where('md5(CAST(assessor_registration_details_fk AS character varying)) =', $insert_id_hash);
		// $this->db->update('council_assessor_registration_application_nubmer',array('ssc_wbsctvesd_certified_flag' => 1);

		$this->db->insert('council_assessor_ssc_wbsctvesd_certified_map', $ssc_wbsctvesd_certified_array_map_table);
	...
	//
	public function get_new_cert_pdf($council_ssc_wbsctvesd_certified_map_id_pk=NULL){
		$query = $this->db->select("toa_certificate")
		->from("council_assessor_ssc_wbsctvesd_certified_map")
		->where("council_ssc_wbsctvesd_certified_map_id_pk",$council_ssc_wbsctvesd_certified_map_id_pk)
		->get();
		return $query->result_array();
	}
...
public function get_work_experiences($app_id_hash = NULL, $assessor_registration_application_no = NULL){
	$query = $this->db->select("*")
	...
	->where("assessor_registration_application_no",$assessor_registration_application_no)
	->get();
return $query->result_array();
...
public function delete_work_exp_dtls($id_hash = NULL)
	...
	public function update_assessor_reg_work_exp_flag($flag = TRUE, $hash_id=NULL)
	{
	 ...
	 public function get_assessor_experience($app_id_hash = NULL,$assessor_registration_application_no){
	...
	public function get_assessor_jobroles($assessor_registration_details_fk = NULL,$assessor_registration_application_no = NULL){
		$query = $this->db->select("map.course_id_fk,map.sector_id_fk,map.job_role_sp_quali,sector.sector_name,map.assessor_registration_application_no")
			...
			->where(
				array(
					"map.active_status" => 1,
					"map.assessor_registration_details_fk" => $assessor_registration_details_fk,
					"map.assessor_registration_application_no" => $assessor_registration_application_no // addition parag 30-03-2021
	...
	public function update_course_data($assessor_main_data,$assessor_id,$jobrole_sector_data,$application_no){
		$this->db->trans_begin();

		$this->db->where('assessor_registration_details_fk', $assessor_id);
		$this->db->where('assessor_registration_application_no', $application_no);
		$this->db->update('council_assessor_registration_application_nubmer', $assessor_main_data); // change

		$this->db->set('active_status', 0);
		$this->db->where('assessor_registration_details_fk', $assessor_id);
		$this->db->where('assessor_registration_application_no', $application_no); // updated/
	...
	public function get_assessor_certificates($assessor_registration_details_fk = NULL,$assessor_registration_application_no = NULL){
		$query = $this->db->select("council_assessor_registration_certified_map_id_pk,assessing_body,certificate_number")
			...
			->where(
				array(
					...
					"active_status" => 1,
					"assessor_registration_application_no" => $assessor_registration_application_no // addition parag 30-03-2021
				)
			)
			->get();
		return $query->result_array();
	...
	public function update_certificate_data($edu_quali_data,$assessor_id,$certificate_data){
		$this->db->trans_begin();

		$this->db->where('assessor_registration_details_fk', $assessor_id);
		$this->db->where('assessor_registration_application_no', $this->input->post("application_no"));
		$this->db->update('council_assessor_registration_application_nubmer', $edu_quali_data);

		$this->db->set('active_status', 0);
		$this->db->where('assessor_registration_details_fk', $assessor_id);
		$this->db->where('assessor_registration_application_no',$this->input->post("application_no"));// 05-04-2021
	...
	public function get_working_with_centre(){
		$query = $this->db->select("*")
			...
			->order_by('sort_order') //Added by Waseem on 27-02-2021
			->get();
		return $query->result_array();
	}
...
//added by Waseem on 27-02-2021
	public function get_css_vse_school_centre($centre_code_hash = NULL){

        $query = $this->db->select('css_vse_id_pk as centre_id,udise_code as centre_code,school_name as working_centre_name')
            ->from('council_css_vse_school_master')
            ->where(
                array(
                    "MD5(CAST(udise_code as character varying)) =" => $centre_code_hash
                )
            )->get();
        return $query->result_array();
	}
	
	public function get_assessor_jobroles_for_assessor_experience($assessor_registration_details_fk = NULL,$assessor_registration_application_no){
		$query = $this->db->select("map.course_id_fk,map.sector_id_fk,map.job_role_sp_quali,sector.sector_name,map.apply_highest_quali,map.domain_exp,map.domain_id_fk")
			...
			->where(
				array(
					"map.active_status" => 1,
					"assessor_registration_details_fk" => $assessor_registration_details_fk,
					"map.assessor_registration_application_no" => $assessor_registration_application_no
				)
			)
			->get();
		return $query->result_array();
	...
	public function get_toa_certificate_file_content($asseaor_id){
		$this->db->select("toa_certificate");
		$this->db->from("council_assessor_registration_details");
		$this->db->where('md5(CAST(assessor_registration_details_pk as character varying)) =', $asseaor_id);
		$query = $this->db->get();
		return $query->result_array();
	...
	//24-03-2021
	public function get_assessor_application_count(){
		$query = $this->db->select("assessor_registration_application_nubmer_id_pk,
			max(assessor_registration_application_no) as assessor_registration_application_no,
			approve_status,
			final_submission_status,
			course_flag,
			edu_qualification_flag,
			work_experience_flag,
			experience_assessor_flag,
			ssc_wbsctvesd_certified_flag,
			highest_qualification_id_pk,
			discipline,
			othert_quali,
			current_emp_status_id_fk,
			certified_by_any_assessor,
			apply_highest_quali,
			domain_exp,
			domain_id_fk,
			apply_for_assessor,
			...
			->limit(1)
			->group_by("assessor_registration_application_nubmer_id_pk,approve_status,final_submission_status,course_flag,
			edu_qualification_flag,
			work_experience_flag,
			experience_assessor_flag,
			ssc_wbsctvesd_certified_flag")
	...
	public function get_courses_by_assessor($assessor_id = NULL, $application_no = NULL){
		$query = $this->db->select("DISTINCT(course.course_id_pk),course.course_name,course.course_code")	
			->from("council_course_master as course")
			->join("council_assessor_registration_jobrole_sector_map as map","course.course_id_pk = map.course_id_fk")
			->where(
				array(
					"map.assessor_registration_details_fk" => $assessor_id,
					"map.assessor_registration_application_no" => $application_no,
					"map.active_status" => 1
	...
	//new module 21-04-2021
	public function assessor_document_upload($document_array = NULL,$assessor_array = NULL, $assessor_id = NULL,$app_no = NULL){
		$this->db->trans_begin();

		$this->db->insert('council_assessor_registration_document_map', $document_array);

		$this->db->where('assessor_registration_details_fk', $assessor_id);
		$this->db->where('assessor_registration_application_no', $app_no);
		$this->db->update('council_assessor_registration_application_nubmer', $assessor_array);
	...
	//new module 21-04-2021
	public function get_assessor_document($application_no = NULL, $assessor_id = NULL){
		$query = $this->db->select("doc.council_assessor_registration_document_map_id_pk,quali.qualification,doc.institute_name,doc.certificate_no")
			->from("council_assessor_registration_document_map as doc")
			->join("council_qualification as quali","doc.qualification_id_fk = quali.qualification_id_pk")
			->where(
				array(
				"doc.assessor_registration_details_fk" => $assessor_id,
				"doc.assessor_registration_application_no" => $application_no,
				"doc.active_status" => 1
				)
			)
			->order_by("doc.council_assessor_registration_document_map_id_pk","ASC")
			->get();
		return $query->result_array();
	}

	public function get_doc_file_content($doc_id_pk){
		$this->db->select("certificate_file");
		$this->db->from("council_assessor_registration_document_map");
		$this->db->where("council_assessor_registration_document_map_id_pk",$doc_id_pk);
		//$this->db->limit($limit,$offset);
		$query = $this->db->get();
		return $query->result_array(); 
