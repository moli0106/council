...
class Questions_model extends CI_Model
{
	...
	//Added on 23-02-2022
	public function getSubjectListByCourseIdSemId($stake_details_id_fk = NULL, $course_id = NULL, $sam_year_id = NULL)
    {
        $this->db->select('subject.subject_id_pk, subject.subject_name, subject.subject_code');
        $this->db->from('council_qbm_question_creator_moderator_subject_map AS QCMSM');
        ...
        $this->db->join('council_qbm_subject_semester_group_trade_map AS SSGTM', 'SSGTM.subject_id_fk = QCMSM.subject_id_fk', 'left');
        $this->db->join('council_qbm_subject_master AS subject', 'subject.subject_id_pk = QCMSM.subject_id_fk', 'left');
		$this->db->join('council_qbm_subject_semester_group_trade_map AS subject_map', 'subject_map.subject_id_fk = subject.subject_id_pk', 'left');

        $this->db->where('QCMSM.creator_moderator_id_fk', $stake_details_id_fk);
        $this->db->where('QCMSM.course_id_fk', $course_id);
		$this->db->where('subject_map.sam_year_id_fk', $sam_year_id);
    ...
    public function insert_eng_question($eng_question_array = NULL)
    {
        $this->db->insert('council_qbm_question_bank_english_lang', $eng_question_array);
        return $this->db->insert_id();
    }
    ...
    public function getSubjectQuestionList($stake_details_id_fk = NULL, $subject_id_hash = NULL)
    {
    ...
    public function getSubjectQuestionListBySem($stake_details_id_fk = NULL, $subject_id_hash = NULL, $semester_id = NULL)
    {
    ...
    public function forwardQuestionSet($stake_details_id_fk = NULL, $subject_id_hash = NULL, $semester_id = NULL)
    {
        ...
        $this->db->update('council_qbm_question_bank', array('process_status_id_fk' => 16));
        return $this->db->affected_rows();
    }

    public function getSubjectQuestionListCount($stake_details_id_fk = NULL, $subject_id_hash = NULL)
    {
    ...
    {
        $this->db->select('QBEL.eng_question_id_pk, QBEL.question, QBEL.question_clue, QBEL.question_pic,QBEL.answer_pic, QBEL.per_question_marks, QBEL.q_type, QB.question_id_pk, QB.subject_id');
    ...
    {
        $this->db->select('SSGTM.sub_cat_id_fk,SSGTM.course_id_fk');
        $this->db->from('council_qbm_subject_semester_group_trade_map AS SSGTM');
