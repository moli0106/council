...
class Questions_qt_model extends CI_Model
{
    ...
    {
        $this->db->select('subject.subject_id_pk, subject.subject_name, subject.subject_code, course.course_name');
        ...
        $this->db->join('council_qbm_course_master AS course', 'course.course_id_pk = QB.course_id', 'left');

        //$this->db->where('QCMSM.creator_moderator_id_fk', $stake_details_id_fk);
        $this->db->where('subject.active_status', 1);
        $this->db->where('QB.active_status', 1);
        $this->db->where('QB.process_status_id_fk', 5);
		$this->db->where_in('QB.course_id', array(1,2));
        $this->db->group_by('subject.subject_id_pk, subject.subject_name, subject.subject_code, course.course_name');

        $this->db->order_by('course.course_name,subject.subject_name');
    ...
    {
        $this->db->select('QBEL.eng_question_id_pk, QBEL.question, QBEL.question_clue, QBEL.question_pic,QBEL.answer_pic, QBEL.per_question_marks, QB.question_id_pk,QB.subject_id');
    ...
    {
        $this->db->select('QBOL.other_question_id_pk, QBOL.question, QBOL.question_clue, QBOL.question_pic,QBOL.answer_pic, QBOL.per_question_marks, QB.question_id_pk,QB.subject_id');
    ...
    {
        $this->db->select('QBEL.eng_question_id_pk, QBEL.question, QBEL.question_clue, QBEL.question_pic,QBEL.answer_pic, QBEL.per_question_marks, QB.question_id_pk, QB.subject_id,QBEL.q_type,QCM.question_category_name,QCM.question_category_id_pk');
    ...
    public function getSubjectCategory($subject_id_hash = NULL)
    {
        $this->db->select('SSGTM.sub_cat_id_fk');
        $this->db->from('council_qbm_subject_semester_group_trade_map AS SSGTM');

        $this->db->where('MD5(CAST("SSGTM"."subject_id_fk" as character varying)) =', $subject_id_hash);
        $this->db->where('SSGTM.active_status', 1);

        return $this->db->get()->result_array();
    }
  

public function getSubjectName($subject_id_hash = NULL)
    {
        ...
        $this->db->where('MD5(CAST("subject"."subject_id_pk" as character varying)) =', $subject_id_hash);
        $this->db->where('subject.active_status', 1);

        return $this->db->get()->result_array();
    }

    public function ApproveQuestionBank($id_hash = NULL, $array = NULL)
    {
        $this->db->where('MD5(CAST("question_id_pk" as character varying)) =', $id_hash);
	...
	public function getEngBengQuestionList($id_hash = NULL)
    {
        $this->db->select('QBEL.eng_question_id_pk, QBEL.question, QBEL.question_clue, QBEL.question_pic,QBEL.answer_pic, QBEL.per_question_marks, QBEL.q_type,
        QBOL.eng_question_id_fk, QBOL.question as beng_question, QBOL.question_clue as beng_question_clue, QBOL.question_pic as beng_question_pic, QBOL.answer_pic as beng_answer_pic,QBOL.per_question_marks as beng_per_question_marks, QBOL.q_type as beng_q_type');

        $this->db->from('council_qbm_question_bank_english_lang AS QBEL');
        $this->db->join('council_qbm_question_bank_other_lang AS QBOL', 'QBEL.eng_question_id_pk = QBOL.eng_question_id_fk', 'left');

        $this->db->where('MD5(CAST("QBEL"."question_id_fk" as character varying)) =', $id_hash);
        $this->db->where('QBEL.active_status', 1);
 ...
 public function insert_download_log_data($data){
        $this->db->insert('council_qbm_download_question_bengali_translator_log',$data);
    }
