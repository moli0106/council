...
class Question_bank_report extends NIC_Controller 
{
    ...
    public function index($offset = 0){
        ...
        if($this->input->method(TRUE) != 'POST'){
           ...
           $total_row = $this->question_bank_report_model->question_bank_report_count()[0]['count'];
           $question_bank_data = $this->question_bank_report_model->get_question_bank_report($per_page,$offset);
           // $data['offset'] = $offset;
        } else {
			...
			if ($this->form_validation->run() == TRUE){
                $question_bank_data 		= $this->question_bank_report_model->question_bank_report_search($this->input->post('sector_id'),$this->input->post('course_id'),$this->input->post('programme_id'),$this->input->post('question_for_id'),$this->input->post('question_type_id'), $per_page,$offset);
                
                $total_row = count($this->question_bank_report_model->question_bank_report_search($this->input->post('sector_id'),$this->input->post('course_id'),$this->input->post('programme_id'),$this->input->post('question_for_id'),$this->input->post('question_type_id')));
            } 
			else {
        ...
        $data['question_bank']  	= $question_bank_data;
