...
class Vacent_college_list extends NIC_Controller
{
    ...
    public function get_spotcouncil_student($college_id_hash = null, $caste_id = null, $discipline_id_fk = null)
    {
        ...
        $data['college_map_id'] = $data['map_details_data']['college_map_id_pk'];
    ...
    public function get_application_no()
    {
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            if (!empty($data['search']) ) {
                $html = $this->load->view($this->config->item('theme') . 'spot_council/ajax_view/spotcouncil_student_details_view', $data, TRUE);
        ...
        if (!empty($data['get_booking'])) {
            ...
            if (($caste_id == 6) && ($data['user_details']['pc_rank'] != NULL)) {
                ...
                if ($student_pc_rank <= min($pc_rank_array)) {
                    ...
                    $msg = 'Oops! Please wait for previous Rank student.';

                    $this->responseMessage($college_id_hash,$assign_student_caste,$clg_discipline_id_fk, $msg);
                ...
                $msg = 'Oops! this PC student can not be admitted on genaral sheet.';
            ...
            //only gen st sc admission on general sheet
                ...
                $msg = 'Oops! this general student can not be admitted on other than genaral sheet.';

                $this->responseMessage($college_id_hash,$assign_student_caste,$clg_discipline_id_fk, $msg);

               
            } else if ($caste_id == 2 && $assign_student_caste == 2  && ($data['user_details']['pc_rank'] == NULL)) {
                ...
                if($getPCVacancy == 0){
                    ...
                    $msg = 'Oops! PC sheets still now available.';
            ...
            }else if(($caste_id == 2) &&($assign_student_caste != 2)) {

                $msg = 'Oops! this other than SC student can not be admitted on Sc sheet.';

                $this->responseMessage($college_id_hash,$assign_student_caste,$clg_discipline_id_fk, $msg);

                    
            }
            else if (($caste_id == 3) && ($assign_student_caste == 3 )  && ($data['user_details']['pc_rank'] == NULL) ) {
            ...
            else if(($caste_id == 3) && ($assign_student_caste != 3)) {

                $msg ='Oops! this other than ST student can not be admitted on ST sheet.';

                $this->responseMessage($college_id_hash,$assign_student_caste,$clg_discipline_id_fk, $msg);
                
            }else{

                $msg ='Oops! citeria not fullfil.';
    ...
    public  function responseMessage($college_id_hash,$assign_student_caste,$clg_discipline_id_fk,$msg){
        $this->session->set_flashdata('status', 'danger');
