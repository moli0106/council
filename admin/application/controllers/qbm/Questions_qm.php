...
class Questions_qm extends NIC_Controller
{
    public function __construct()
    {
        parent::__construct();
        parent::check_privilege(96);
        //$this->output->enable_profiler(TRUE);
        $this->load->model('qbm/questions_qm_model');

        $this->css_head = array(
            1 => $this->config->item('theme_uri') . 'bower_components/select2/dist/css/select2.min.css',
        );
        $this->js_foot = array(
            1 => $this->config->item('theme_uri') . "qbm/question_qm.js",
    ...
    public function index($offset = 0)
    {
        ...
        if (!empty($data['subject_id_hash'])) {
            ...
            $data['semesterList'] = $this->questions_qm_model->getSemesterListBySubjectId($data['subject_id_hash']);
          
            $data['questionListCount'] = $this->questions_qm_model->getSubjectQuestionListCount($stake_details_id_fk, $data['subject_id_hash'])[0]['count'];
            $data['questionList'] = $this->questions_qm_model->getSubjectQuestionList($stake_details_id_fk, $data['subject_id_hash']);


            $data['questionCategoryList'] = $this->questions_qm_model->getSubjectQuestionCategoryList($data['subject_id_hash']);

            $this->load->view($this->config->item('theme') . 'qbm/question_moderator/question_list_view', $data);
        } else {
            redirect(base_url('admin/qbm/questions_qm/subjects'));
        }
    }
    ...
    public function getQuestionListBySemSubj()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            if (!empty($subject_id_hash) && !empty($semester_id)) {
                ...
                $questionList = $this->questions_qm_model->getSubjectQuestionListBySem($stake_details_id_fk, $subject_id_hash, $semester_id);
                //$questionCategoryList = $this->questions_qm_model->getSubjectQuestionCategoryList($subject_id_hash);
				$questionCategoryList = $this->questions_qm_model->getSubjectQuestionCategoryList_by_sem($subject_id_hash,$semester_id);   // Added by Waseem on 07-02-2022
                        ...
                        $htmlView_questionList .= '
                            <tr id="' . md5($question['question_id_pk']) . '">
                                ...
                                <td>
                                    ...
                                    ' . $muli_button . '
                                    <button class="btn btn-danger btn-xs btn-flat remove-question">Reject</button>
                                </td>
                            </tr>
                        ';
                    }
    ...
    public function forwardQuestionSet($semester_id = NULL, $subject_id_hash = NULL)
    {
        ...
        $this->questions_qm_model->forwardQuestionSet($stake_details_id_fk, $subject_id_hash, $semester_id);
    ...
    public function question_details($id_hash = NULL)
    {
        if (!$this->input->is_ajax_request()) {
            ...
            if (!empty($id_hash)) {
                ...
                $data['question_details'] = $this->questions_qm_model->getEngQuestionList($id_hash);
                $data['question_category'] = $this->questions_qm_model->getQuestionCategory();
                if (!empty($data['question_details'])) {

                    $html_view = $this->load->view($this->config->item('theme') . 'qbm/question_moderator/ajax/question_details_view', $data, TRUE);
    ...
    public function addMultiLangQuestion_old($id_hash = NULL)
    {
        if (!$this->input->is_ajax_request()) {
            if ($this->input->server('REQUEST_METHOD') == 'POST') {
                ...
                $question_details = $this->questions_qm_model->getEngQuestionList($id_hash);
                $other_question_details = $this->questions_qm_model->getOtherQuestionList($id_hash);
                ...
                foreach ($data['question'] as $key => $question) {
                    ...
                    if (!empty($question) && !empty($data['question_clue'][$key])) {
                        ...
                        if (!empty($other_question_details)) {
                            ...
                            $this->questions_qm_model->update_other_question($data['other_question'][$key], $other_question_array);
                        } else {
                            ...
                            $this->questions_qm_model->insert_other_question($other_question_array);
                        }
                    } else {
                        ...
                        redirect(base_url('admin/qbm/questions_qm?sub=' . md5($question_details[0]['subject_id'])));
                        exit();
                    }
                }

                $this->questions_qm_model->updateQuestionBank($question_details[0]['question_id_pk'], array("other_lan_quesstion_status" => 1));
            ...
            if (!empty($id_hash)) {
                ...
                $data['question_details'] = $this->questions_qm_model->getEngQuestionList($id_hash);
                $data['other_question_details'] = $this->questions_qm_model->getOtherQuestionList($id_hash);

                if (!empty($data['question_details'])) {

                    $data['question_category'] = $this->questions_qm_model->getQuestionCategory();

                    $html_view = $this->load->view($this->config->item('theme') . 'qbm/question_moderator/ajax/question_add_multi_lang_view', $data, TRUE);
            ...
            if ($this->input->server('REQUEST_METHOD') == 'POST') {
                ...
                foreach ($data['question'] as $key => $question) {
                    ...
                    if ($file_validation_err == 0) {
                        ...
                        redirect(base_url('admin/qbm/questions_qm/'));
                    }
                }
    ...
    public function updateEngQuestion($id_hash = NULL)
    {
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            foreach ($data['question'] as $key => $question) {
                ...
                $this->questions_qm_model->updateEngQuestionBank($eng_question_id_hash, $updateArray);
            }
    ...
    public function eng_question_image($id_hash = NULL)
    {
        if (!empty($id_hash)) {
            ...
            $eng_question_details = $this->questions_qm_model->getEngQuestionDetails($id_hash);
    ...
    public function other_question_image($id_hash = NULL)
    {
        if (!empty($id_hash)) {
            ...
            $eng_question_details = $this->questions_qm_model->getOtherQuestionDetails($id_hash);
            if (!empty($eng_question_details)) {
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            if (!empty($id_hash)) {
                ...
                if (!empty($question_details)) {
                    ...
                    $update_status = $this->questions_qm_model->removeQuestionBank($id_hash, array('process_status_id_fk' => 6));
                    if ($update_status) {
    ...
    {
        $stake_details_id_fk = $this->session->userdata('stake_details_id_fk');
        $data['courseList'] = $this->questions_qm_model->getCourseListWithSemester($stake_details_id_fk);

        $this->load->view($this->config->item('theme') . 'qbm/question_moderator/course_list_view', $data);
    }
    ...
    public function subjects()
    {
        ...
        $data['subjectList'] = $this->questions_qm_model->getCreatorSubjectList($stake_details_id_fk);

        // parent::pre($data);
        $this->load->view($this->config->item('theme') . 'qbm/question_moderator/subject_list_view', $data);
    }

    public function add()
    {
        $stake_details_id_fk = $this->session->userdata('stake_details_id_fk');
        $data['courseList'] = $this->questions_qm_model->getCourseList($stake_details_id_fk);

        $this->load->view($this->config->item('theme') . 'qbm/question_moderator/question_add_view', $data);
    }
    ...
    public function create()
    {
        ...
        if ($this->input->server("REQUEST_METHOD") == "POST") {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $question_id = $this->questions_qm_model->insert_question_bank($question_bank_array);
                if ($question_id) {
                    ...
                    foreach ($data['question'] as $key => $question) {
                        ...
                        if ($file_validation_err == 0) {
                            ...
                            $this->questions_qm_model->insert_eng_question($eng_question_array);
                        } else {
                            $this->db->trans_rollback();
    ...
    public function get_semester_subject_list()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            if (!empty($course_id)) {
                ...
                $semester_list = $this->questions_qm_model->getSemesterList($stake_details_id_fk, $course_id);
                $subject_list = $this->questions_qm_model->getSubjectListByCourseId($stake_details_id_fk, $course_id);
    ...
    public function get_disciplines()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            if (!empty($course_id)) {
                ...
                $discipline_list = $this->questions_qm_model->getDisciplineList($stake_details_id_fk, $course_id);
                $semester_list = $this->questions_qm_model->getSemesterList($stake_details_id_fk, $course_id);
    ...
    public function get_subject()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            if (!empty($discipline_id) && !empty($sam_year_id)) {
                $subject_list = $this->questions_qm_model->getSubjectList($course_id, $stake_details_id_fk, $discipline_id, $sam_year_id);
    ...
    public function get_group_trade()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            if (!empty($discipline_id)) {
                $group_trade_list = $this->questions_qm_model->getGroupTradeList($stake_details_id_fk, $discipline_id);
    ...
    public function get_subject_group_category()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            $subject_group_list = $this->questions_qm_model->getSubjectGroupCategory();
    ...
    public function get_subject_by_subject_group_trade()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            if (!empty($group_trade_id) && !empty($subject_group_id)) {
                $subject_list = $this->questions_qm_model->getSubjectBySubjectGroupTrade($group_trade_id, $subject_group_id);
    ...
    public function get_topic_chapter()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            if (!empty($subject_id)) {
                $topic_list = $this->questions_qm_model->getTopicChapterList($subject_id, $sam_year_id);
                $subjectQuestionTypeMark = $this->questions_qm_model->getSubjectQuestionTypeMark($subject_id,$sam_year_id);
        ...
        if (!empty($id_hash)) {
            ...
            if (!empty($eng_question_details)) {
                ...
                if($question_pic !=''){
                    ...
                    $this->questions_qm_model->updateEngQuestionImage(md5($eng_question_details[0]['eng_question_id_pk']), $updateArray);

                    $this->session->set_flashdata('status', 'success');
                    $this->session->set_flashdata('alert_msg', 'Image has been transfered successfully.');
                    redirect(base_url('admin/qbm/questions_qm?sub=' . md5($eng_question_details[0]['subject_id'])));
                    exit();
        ...
        if (!empty($id_hash)) {
            ...
            if (!empty($eng_question_details)) {
                ...
                if($question_pic !=''){
                    ...
                    $this->questions_qm_model->updateotherQuestionImage(md5($eng_question_details[0]['other_question_id_pk']), $updateArray);
