...
class Question_creator_moderator extends NIC_Controller {
		    ...
		    if ($this->form_validation->run() == FALSE) {
                ...
                $this->load->view($this->config->item('theme').'qbm_master/question_creator_moderator/question_creator_moderator_add_view', $data);
                    ...
                    if($last_id)
                    {
                        ...
                        $email_subject = "Credentials For Question Creator/Moderator Account";
    ...
    public function get_subject($course_id)
	{
		...
		{
		$data['subjectList'] = $this->question_creator_moderator_model->get_subject_List($course_id);
	...
	public function excel_download()
	{
		...
		$fileName = 'qcm_details_report_report_'.date('Yms').'.xls';
		...
		$objPHPExcel->getActiveSheet()->SetCellValue('A1', 'Sl No')							 							 
                                      ->SetCellValue('B1', 'Question Creator/Modurator name')
									  ->SetCellValue('C1', 'Question Creator/Modurator Type')
									  ->SetCellValue('D1', 'Creator/Moderator ID')
		...
		$excel_data  	= $this->question_creator_moderator_model->get_qcm_details_report();
        ...
        foreach($excel_data as $assessor)
        {
        	...
        	$objPHPExcel->getActiveSheet()->SetCellValue('B'.$row, $assessor['qc_name']);
        	$objPHPExcel->getActiveSheet()->SetCellValue('C'.$row, $creator_moderator_type);
        	$objPHPExcel->getActiveSheet()->SetCellValue('D'.$row, $assessor['creator_moderator_code']);
        	$objPHPExcel->getActiveSheet()->SetCellValue('E'.$row, $assessor['email_id']);
        	$objPHPExcel->getActiveSheet()->SetCellValue('F'.$row, $assessor['mobile_no']);
			$objPHPExcel->getActiveSheet()->SetCellValue('G'.$row, $assessor['course_name']);
        	$objPHPExcel->getActiveSheet()->SetCellValue('H'.$row, $assessor['subject_name']);
        	$objPHPExcel->getActiveSheet()->SetCellValue('I'.$row, $assessor['subject_code']);
	...
	public function password_mail($qcmid=NULL)
	{
		...
		$qcm_details = $this->question_creator_moderator_model->get_qcm_email($qcmid)[0];
        
        $creator_moderator_id_pk=$qcm_details['creator_moderator_id_pk'];
        $qcm_email_id=$qcm_details['email_id'];
        $mobile_no=$qcm_details['mobile_no'];
        $creator_moderator_type=$qcm_details['creator_moderator_type'];
		$login_dtls = $this->question_creator_moderator_model->get_qcm_login_dtls($creator_moderator_id_pk,$creator_moderator_type);
		...
		{
			$send=1;
			$email_status=send_email($qcm_email_id, $email_message, $email_subject);
		}
