...
class Topics extends NIC_Controller {

    public function __construct()
    {
        parent::__construct();
        parent::check_privilege(86);
        // $this->output->enable_profiler(TRUE);
        $this->load->model('qbm_master/topics_model');
    ...
    public function index( $offset = 0 )
    {
        ...
        $config['base_url']         = 'qbm_master/topics/index/';
		$data["total_rows"]         = $config['total_rows'] = $this->topics_model->get_subjectCount()[0]['count'];	
		$config['per_page']         = 25;
        $config['num_links']        = 2;
        ...
        $data['subjectList'] = $this->topics_model->getAll_subject($config['per_page'], $offset);

        $this->load->view($this->config->item('theme').'qbm_master/topics_list_view', $data);
    }
    ...
    {
        $data = array(
            'course_list' => $this->topics_model->getAllcourse(),
            'sub_group_list' => $this->topics_model->getAllsubGroup()
        );
        ...
        if($this->input->server('REQUEST_METHOD') == "POST")
        {
		    ...
		    if ($this->form_validation->run() == FALSE) {
                if(set_value('course_id') != NULL){
                    $data['disciplineList'] = $this->topics_model->getDisciplineList(set_value('course_id'));
                    $data['semesterList'] = $this->topics_model->get_semester_List(set_value('course_id'));
                }
                if(set_value('discipline_id') != NULL){
                    $data['group_tradeList'] = $this->topics_model->get_group_trade_List(set_value('discipline_id'));
                }
                $this->load->view($this->config->item('theme').'qbm_master/topics_add_view', $data);
                ...
                if($this->input->post('course_id')==3 || $this->input->post('course_id')==4){
                    ...
                    $last_id = $this->topics_model->insert_subject_data($post_data);
                    ...
                    redirect('admin/qbm_master/topics');
    ...
    public function get_discipline($course_code)
	{
		...
		{
		$data['disciplineList'] = $this->topics_model->getDisciplineList($course_code);
    ...
    public function get_group_trade($discipline_id)
	{
		...
		{
		$data['group_tradeList'] = $this->topics_model->get_group_trade_List($discipline_id);
    ...
    public function get_semester($course_code)
	{
		if(is_numeric($course_code))
		{
		$data['semesterList'] = $this->topics_model->get_semester_List($course_code);
