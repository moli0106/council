...
class Discipline extends NIC_Controller {
    ...
    public function index( $offset = 0 )
    {
		...
		$data["total_rows"]         = $config['total_rows'] = $this->discipline_model->get_semesterCount()[0]['count'];	
		$config['per_page']         = 25;
        $config['num_links']        = 2;
        ...
        $data['disciplineList'] = $this->discipline_model->getAll_semester($config['per_page'], $offset);
    ...
    public function add()
    {
        ...
        if($this->input->server('REQUEST_METHOD') == "POST")
        {
		    ...
		    if ($this->form_validation->run() == FALSE) {
                ...
                $this->load->view($this->config->item('theme').'qbm_master/discipline_add_view', $data);
