...
class Courses extends NIC_Controller
{
    ...
    //     $data['vtcCourseList']  = $this->course_model->getVtcAllCourseList($data['vtc_id'], $data['academic_year']);
    //     // echo "<pre>";print_r($data['vtcCourseList']);exit;
    //     $this->load->view($this->config->item('theme') . 'affiliation/courses_list_view', $data);
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                if ($courseSelectionId) {
                    ...
                    // $data['course'] = $this->course_model->getCourseDetailsById(md5($courseSelectionId));
                    // $data['courseSubject'] = $this->course_model->getSubjectNameByVTCId(md5($courseSelectionId));
                    ...
                    // redirect('admin/affiliation/courses/new_course');
        ...
        if (!$this->input->is_ajax_request()) {
            ...
            if(!empty($courseName) && $courseName!= 'undefined'){
                ...
                if($courseName == 1){
                    ...
                    $data['groupList'] = $this->course_model->getCourseListByNameDisciplineAndStreem($courseName, $discipline, $hs_science, $hs_biology, $hs_comerce, $vtc_id, $academic_year);
                    ...
                    $data['nqrCourseList']  = $this->course_model->getCourseListByNameDiscipline(2, $discipline,$vtc_id, $academic_year);
                    $data['nsqfCourseList'] = $this->course_model->getCourseListByNameDiscipline(3, $discipline,$vtc_id, $academic_year);
                    $data['viiXCourseList'] = $this->course_model->getCourseListByNameDiscipline(5, $discipline,$vtc_id, $academic_year);
                    
                    // echo "<pre>";print_r($data['nqrCourseList']);exit;
    ...
    public function getGroupCount(){
        if(!$this->input->is_ajax_request()){
            ...
            if(!empty($group_id) && !empty($course_name_id)){
                ...
                if($course_name_id == 1){ // select unlimited course for VII+STC add on 21-10-2022
    ...
    public function resetAllCourse()
    {
        if (!$this->input->is_ajax_request()) {
            ...
            $this->course_model->resetStudent($vtcDetails['vtc_id_pk'], $data['academic_year']);

            // if($vtcDetails['second_final_submit_status']== 1){

                $this->course_model->resetLaboratoryAndAgriDetails($vtcDetails['vtc_id_pk'], $vtcDetails['academic_year']);
            // }
