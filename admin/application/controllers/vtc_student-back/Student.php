...
class Student extends NIC_Controller
{
    ...
    public function index()
    {
       $data['school_reg_id_pk'] =  $this->session->userdata('stake_details_id_fk');
        ...
        $data['vtcDetails'] = $this->student_model->getVtcDetails($data['school_reg_id_pk'], $data['academic_year']);
        ...
        $data['yearlist']  = $this->student_model->getAcademicYearList();
    ...
    public function add()
    {
        ...
        if (!empty($data['school_data'])) {
            ...
            if ($this->input->server("REQUEST_METHOD") == "POST") {
                ...
                $data['student_data']['stdMobile'] = $this->input->post('stdMobile');
    ...
    public function get_list()
    {
        ...
        $search = $this->input->GET('search')['value'];
            ...
            if (!empty($search)) {
                ...
                $data['studentList']     = $this->student_model->getStudentListBySearch($limit, $start,$orderColumn, $orderType, $search,$selected_year);
            } else {

                $data['studentList']     = $this->student_model->getStudentListByVTCId($limit, $start, $orderColumn, $orderType,$selected_year,$vtc_id);
            }
            ...
            foreach ($data['studentList'] as $data) {
                ...
                $options1 = '<a class="btn btn-info btn-xm" title = Details href="vtc_student/student/student_details/'. md5($data['student_id_pk']) .'" ><i class="fa fa-folder-open-o" aria-hidden="true"></i></a>';
            ...
            $output = array(
                ...
                "recordsTotal" => $this->student_model->getStudentListCount($selected_year)[0]['count'],
                "recordsFiltered" => $this->student_model->getStudentListCount($selected_year)[0]['count'],
                "data" => $info
            );
            } 
            else {
    ...
    public function add_student(){
        ...
        $data['school_reg_id_pk'] =  $this->session->userdata('stake_details_id_fk');
        $data['academic_year'] = $this->config->item('current_academic_year');
        $data['school_data'] = $this->student_model->getVtcDetails($data['school_reg_id_pk'], $data['academic_year']);

        if (!empty($data['school_data'])) {

            

            $data['salutations'] =  $this->student_model->getSalutation();
            $data['genders'] =  $this->student_model->getGender();
            $data['districtList']  = $this->student_model->getDistrictList();
            $data['casteList']  = $this->student_model->get_caste();
            $data['religion']  = $this->student_model->get_religion();
            $data['last_exam']  = $this->student_model->get_last_exam();
            $data['batchDurationList']  = $this->student_model->get_batch_duration();
            $data['academic_year']  = $this->config->item('current_academic_year');

            $data['stateList']  = $this->student_model->getAllState();
            $data['nationality']  = $this->student_model->get_nationality();
            $data['boardList']  = $this->student_model->getAllboard();
            ...
            if ($this->input->server('REQUEST_METHOD') == 'POST') {
                ...
                if (!empty($data['form_data']['state'])) {
                    $data['district'] = $this->student_model->getDistrictByStateId($data['form_data']['state']);
                }
                ...
                if(!empty($course_name_id)){
                    ...
                    $data['group'] = $this->student_model->getGroupByVTCCode($course_name_id,$vtc_code, $data['academic_year']);
                }

                $group_id = $this->input->post('group_id');

                if(!empty($group_id)){
                    $data['group_details'] = $this->student_model->getGroupDetails($group_id);
                }
                ...
                if ($this->form_validation->run() == FALSE) {
                    ...
                    $std_id = $this->student_model->insertData('council_vtc_student_master', $std_data);
                    ...
                    if ($std_id) {
                        ...
                        $result = $this->student_model->insertData('council_vtc_student_last_examination', $academicData);
                        ...
                        redirect('admin/vtc_student/student/add_student');
    ...
    public function is_unique_aadhar_no($aadhar_no){
		...
		{
			$duplicate_no = $this->student_model->get_duplicate_aadhar($aadhar_no);
    ...
    public function getGroupName($course_name_id = NULL)
    {
        if (!$this->input->is_ajax_request()) {
            ...
            if($course_name_id !='' && $vtc_code !=''){
                ...
                $group = $this->student_model->getGroupByVTCCode($course_name_id,$vtc_code, $academic_year);
    ...
    public function student_details($id_hash = NULL){
        ...
        if (!empty($data['form_data']['class_id_fk'])) {
            $data['group'] = $this->student_model->getGroupByVTCCode($data['form_data']['class_id_fk'],$data['school_data']['vtc_code'], $data['academic_year']);
        }

        $group_id = $this->input->post('group_id');

        if(!empty($data['form_data']['course_id_fk'])){
            $data['group_details'] = $this->student_model->getGroupDetails($data['form_data']['course_id_fk']);
        }else{
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            if ($this->form_validation->run() == FALSE) {
                ...
                $result = $this->student_model->updateStudentData($id_hash, $update_data);
                ...
                if ($result) {
                    ...
                    $result_1 = $this->student_model->updateStudentAcademicData($id_hash, $academicData);
    ...
    public function updateStudentApproveRejectStatus(){
        ...
        if ($this->input->server('REQUEST_METHOD') == "POST") {
            ...
            if($status == 0){
                ...
                $rejectStatus = $this->student_model->updateStudentData($std_id_hash,$updArray);
                ...
                redirect(base_url('admin/vtc_student/student'));
                ...
                $approveStatus = $this->student_model->updateStudentData($std_id_hash,$updArray);
    ...
    public function getDuration($course_name_id = NULL)
    {
        if (!$this->input->is_ajax_request()) {
            ...
            if($course_name_id !='' && $group_id !=''){
                ...
                $group = $this->student_model->getGroupDetails($group_id);
    ...
    public function getDistrict($state_id = NULL)
    {
        if (!$this->input->is_ajax_request()) {
            ...
            $district = $this->student_model->getDistrictByStateId($state_id);
    ...
    public function showPdfDoc($id_hash = NULL , $type = NULL){
        ...
        if (!empty($id_hash)) {
            ...
            $student_data = $this->student_model->getStudentDetailsById($id_hash);
            if (!empty($student_data)) {
                ...
                redirect('admin/vtc_student/student/student_details/'.$id_hash);
            }
        } else {
            redirect('admin/vtc_student/student/student_details/'.$id_hash);
        }
    ...
    public function test_api(){
		...
		curl_setopt_array($curl, array(
		  ...
		  CURLOPT_POSTFIELDS => 'StudentCode=06604719002823&Src=VTC',
