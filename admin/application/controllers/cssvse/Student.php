...
class Student extends NIC_Controller
{
    function __construct()
    {
        ...
        $this->js_foot = array(
            ...
            2 => $this->config->item('theme_uri') . 'cssvse/student.js',
        );
    }
    ...
    public function index()
    {
        ...
        $this->load->view($this->config->item('theme') . 'cssvse/student/student_list_view', $data);
    }
        ...
        if (!empty($data['school_data'])) {
            ...
            if ($this->input->server("REQUEST_METHOD") == "POST") {
                ...
                if ($this->form_validation->run() != FALSE) {
                    ...
                    redirect('admin/cssvse/student/add');
                } else {
            ...
            $this->load->view($this->config->item('theme') . 'cssvse/student/student_add_view', $data);
        }
    }
    ...
    public function details($id_hash = NULL)
    {
        if (!$this->input->is_ajax_request()) {
            ...
            if (!empty($data['studentDetails'])) {
                ...
                $html_view = $this->load->view($this->config->item('theme') . 'cssvse/student/ajax/student_details_view', $data, TRUE);
    ...
    public function update()
    {
        if ($this->input->server("REQUEST_METHOD") == "POST") {
            ...
            $this->form_validation->set_rules('student_image', 'Student Image', 'trim|callback_file_validation[student_image|image/jpeg|100|]');
