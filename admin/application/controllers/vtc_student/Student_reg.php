...
class Student_reg extends NIC_Controller
{
    ...
    public function index()
    {
       $data['school_reg_id_pk'] =  $this->session->userdata('stake_details_id_fk');
		...
		//added by moli 01-06-2023
        $select_batch_no = $this->input->post('batch_no'); 
        if($select_batch_no == ''){
            $data['batch_no']  = null;
        }else{
            $data['batch_no']  =$select_batch_no;
        }
    ...
    public function get_list()
    {
        ...
        $search = $this->input->GET('search')['value'];
            ...
            if (!empty($search)) {
                ...
                $data['studentList']     = $this->student_reg_model->getStudentListBySearch($limit, $start,$orderColumn, $orderType, $search,$selected_year,$vtc_id , $batch_no);
				
				$listCount = count($this->student_reg_model->getStudentListBySearch($limit, $start,$orderColumn, $orderType, $search,$selected_year,$vtc_id));
            } else {

                $data['studentList']     = $this->student_reg_model->getStudentListByVTCId($limit, $start, $orderColumn, $orderType,$selected_year,$vtc_id,$batch_no);
				$listCount = $this->student_reg_model->getNewStudentListCount($selected_year,$vtc_id,$batch_no)[0]['count'];
				//$listCount = count($data['studentList']);
            }
			
			//$data['std_count'] = $this->student_reg_model->getNewStudentListCount($selected_year,$vtc_id);
            ...
            foreach ($data['studentList'] as $data) {
				...
				// added on 15-03-2023
                if($data['payment_status'] == "No") {
                    $payment = 'No';
					$options3 = '';
				}else{
					$payment = 'Yes';
					$options3 = '<a class="btn btn-success btn-xm" title = Preview href="vtc_student/student_reg/student_preview/'. md5($data['student_id_pk']) .'" ><i class="fa fa-print" aria-hidden="true"></i></a>';
					//$option3 = 'yes';
				}
                ...
                if ($data['payment_status'] == 'Yes' && $data['eligible_for_exam'] != 1){
                    //$check_button = '<input type="checkbox" name="std_id_pk" class="checkStd" value="'.$data['student_id_pk'].'">';
					$check_button = '';
                ...
                if ($data['eligible_for_exam'] == 1){

                    

                    $exam_status = '<small class="label label-info">Eligible</small>';
                ...
                $nestedData['batch'] =$batch_phase;
                $nestedData['course_name'] = $data['course_name'];
                $nestedData['group_name'] =  $data['group_name'];
                $nestedData['group_code'] =  $data['group_code'];
                $nestedData['status'] =  $status .'<br>'.$exam_status;
				$nestedData['payment_status'] =  $payment;
                $nestedData['action'] = $options1.' '.$options2.' '.$options3;
        ...
        if (!empty($data['school_data'])) {
            ...
            if ($this->input->server('REQUEST_METHOD') == 'POST') {
                ...
                $config = array(
                    ...
                    array('field' => 'aadhar_no','label' => 'Aadhar Number','rules' => 'trim|required|exact_length[12]|numeric|callback_is_unique_aadhar_no'),
					...
					array('field' => 'kanyashree_no','label' => 'Kanyashree Number','rules' => 'trim|exact_length[20]|is_unique[council_vtc_student_master.kanyashree_no]')
                ...
                if ($this->form_validation->run() == FALSE) {
					...
					//Added by moli on 06-04-2023
                    $getassessmentCourse = $this->student_reg_model->getassessmentCourseSectorId($this->input->post('group_id'));
                    if($getassessmentCourse){
                        $assessment_course_id_fk = $getassessmentCourse['assessment_course_id_fk'];
                        $assessment_sector_id_fk = $getassessmentCourse['assessment_sector_id_fk'];
                    }else{
                        $assessment_course_id_fk = NULL;
                        $assessment_sector_id_fk = NULL;
                    }
					
					//Added By Moli on 22-05-2023
                    $getBatchNo = $this->student_reg_model->getBatchNoByGroupId($this->input->post('group_id'),$data['school_data']['vtc_id_pk'],$data['academic_year']);
                    ...
                    $std_data = array(
                        ...
                        'middle_name'           => ($this->input->post('mname') != NULL) ? strtoupper($this->input->post('mname')) : NULL,
                        'last_name'             => strtoupper($this->input->post('lname')),
                        'father_name'           => ($this->input->post('father_name') != NULL) ? strtoupper($this->input->post('father_name')) : '',
                        'mothers_name'           => ($this->input->post('mother_name') != NULL) ? strtoupper($this->input->post('mother_name')) : '' ,
						...
						'assessment_course_id_fk'   => $assessment_course_id_fk,
                        'assessment_sector_id_fk'  => $assessment_sector_id_fk,
						
						'batch_phase'           => $getBatchNo['batch_no']
    ...
    public function getGroupName($course_name_id = NULL)
    {
        if (!$this->input->is_ajax_request()) {
            ...
            if($course_name_id !='' && $vtc_code !=''){
				...
				//added by moli on 22-05-2023 for only declare batch
				$group = $this->student_reg_model->getBatchDeclaredGroup($course_name_id,$vtc_code, $academic_year);
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
			...
			$getBatchNo = $this->student_reg_model->getBatchNoByGroupId($this->input->post('group_id'),$data['school_data']['vtc_id_pk'],$data['academic_year']);
                ...
                if ($result) {
                    ...
                    redirect('admin/vtc_student/student_reg','refresh');
    ...
    public function getDistrict_old($state_id = NULL)
    {
        if (!$this->input->is_ajax_request()) {
            exit('No direct script access allowed');
        } else {
	...
	// added by on 15-03-2023
    public function student_preview($id_hash = NULL){
		//echo $id_hash;exit;
        ...
        $data['academic_year'] = $this->config->item('current_academic_year');
        $data['school_data'] = $this->student_reg_model->getVtcDetails($data['school_reg_id_pk'], $data['academic_year']);
		//echo "<pre>";print_r($data['school_data']);exit;
        $data['student_data'] = $this->student_reg_model->getStudentPreviewDetailsById($id_hash);
		  
        $this->load->view($this->config->item('theme') . 'vtc_student/student/student_profile_view', $data);
	}
	
	// added by moli on 20-03-2023
	public function download_consolidated_list(){
		//echo "hii";exit;
        $data['school_reg_id_pk'] =  $this->session->userdata('stake_details_id_fk');
        $data['academic_year'] = $this->config->item('current_academic_year');
        $data['vtcDetails'] = $this->student_reg_model->getVtcDetails($data['school_reg_id_pk'], $data['academic_year']);
		
        $data['student_data'] = $this->student_reg_model->getAllStudentByVTCId($data['school_reg_id_pk'],$data['academic_year']);
		
         //echo "<pre>";print_r($data);exit;
        if($data['student_data']){

           $html = $this->load->view($this->config->item('theme') . 'vtc_student/student/consolidated_list_pdf_view', $data,true);
    
           $pdfFilePath = 'VTC-Consolidated-List-' . date('dmY') . ".pdf";
           ...
           $this->m_pdf->pdf->Output($pdfFilePath, "I");
            ...
            redirect('admin/vtc_student/student_reg');
        }
    }
	
	// added by moli on 11-04-2023
    public function readyForExam(){
        if (!$this->input->is_ajax_request()) {
            exit('No direct script access allowed');
        } else {
            ...
            if($stdIdArray){
                ...
                $status = $this->student_reg_model->updateStdEligibility($stdIdArray,$upd_data);
                if($status){
                    echo json_encode('done');
                }
            }
	...
	//added by moli on 17-05-2023
    public function checkRegNo($group_id = NULL){
        if (!$this->input->is_ajax_request()) {
            exit('No direct script access allowed');
        } else {
            ...
            if($group_id !=''){
                ...
                $reg_no = $this->student_reg_model->checkRegNoExist($group_id);
