...
class New_domain extends NIC_Controller 
{
	function __construct()
	{
		parent::__construct();
        parent::check_privilege(8);
        $this->load->model('master/new_domain_model');
        ...
        $this->js_foot = array(
            ...
            2 => $this->config->item('theme_uri')."master/new_domain.js",
        );
    ...
    public function index($offset = 0){
		...
		$config['base_url']         = 'master/new_domain/index/';
		$data["total_rows"] = $config['total_rows']       = $this->new_domain_model->get_domain_count()[0]['count'];	
		$config['per_page']         = 100;
        $config['num_links']        = 2;
        ...
        $this->form_validation->set_rules('domain_name', '<b>Domain name</b>', 'trim|required|max_length[100]|is_unique[council_sector_master.sector_name]');

        //print_r($data['sectors']);
        

        if ($this->form_validation->run() == FALSE) {
            $data['domains']  	= $this->new_domain_model->get_domain($config['per_page'],$offset);
            $this->load->view($this->config->item('theme').'master/new_domain_view',$data);
        }  else {
            
           $insert_array = array(
               'table_name' => 'council_domain_master',
               'data'       => array(
           ...
           if($this->new_domain_model->insert($insert_array)){
                $data['status'] = 'success';
                $data['message'] = "New Domain has been successfully inserted";
                $data['domains']  	= $this->new_domain_model->get_domain($config['per_page'],$offset);
    ...
    public function delete_domain_details($domain_id_hash = NULL,$action = NULL){
        $data['domain_id_hash'] = $domain_id_hash;
        $data['course'] = array();

        //$data['course'] = $this->new_domain_model->get_single_course($course_id_hash);
        if($action == "delete"){
            $this->new_domain_model->delete_domain($domain_id_hash);
        } else {
            $data['course'] = $this->new_domain_model->get_single_domain($domain_id_hash);
        }
        
        $this->load->view($this->config->item('theme').'master/delete_domain_view',$data);
    }
