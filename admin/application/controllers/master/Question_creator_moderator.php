...
class Question_creator_moderator extends NIC_Controller {
    ...
    {
        parent::__construct();
        parent::check_privilege(23);
        // $this->output->enable_profiler(TRUE);
        $this->load->model('master/question_creator_moderator_model');
    ...
    public function index( $offset = 0 )
    {
        ...
        $this->load->view($this->config->item('theme').'master/question_creator_moderator/question_creator_moderator_list_view', $data);
    }
        ...
        if($this->input->server('REQUEST_METHOD') == "POST")
        {
            ...
            $config = array(
                ...
                array(
                    'field' => 'email',
                    'label' => 'Email',
                    'rules' => 'trim|required|valid_email|is_unique[council_question_creator_moderator_details.email_id]'
                ),
                array(
                    'field' => 'mobile',
                    'label' => 'Mobile Number',
                    'rules' => 'trim|required|max_length[10]|min_length[10]|regex_match[/^[0-9]{10}$/]|is_unique[council_question_creator_moderator_details.mobile_no]'
                ),
                array(
                    'field' => 'designation',
                    'label' => 'Designation',
		    ...
		    if ($this->form_validation->run() == FALSE) {
                ...
                $this->load->view($this->config->item('theme').'master/question_creator_moderator/question_creator_moderator_add_view', $data);
				...
				$sectorIds = $this->input->post('sector_id[]');
                    ...
                    {
						$mapArray = array();
                        foreach ($sectorIds as $key => $sectorId) 
                        {
                            $mapArray[] = array(
                                'creator_moderator_id_fk' => $last_id,
                                'sector_id_fk'         => $sectorId,
                        ...
                        $this->question_creator_moderator_model->map_question_creator_moderator_sector($mapArray);
						
                        // Create Master Trainer Credentials
                        if($this->input->post('creator_moderator_type')==6){
                            $qcmt='QC';
                            $qcm='P';
						...
						$creator_moderator_code= $qcm.sprintf("%05d", $last_id);
