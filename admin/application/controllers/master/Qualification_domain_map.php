...
class Qualification_domain_map extends NIC_Controller 
{
    ...
    public function searchQualificationDomainMap()
    {
        $qualification = $this->input->post('search_qualification');
        $domain        = $this->input->post('search_domain');

        $this->session->set_flashdata('search_qualification', $qualification);
        $this->session->set_flashdata('search_domain', $domain);
        ...
        if($qualification || $domain)
        {
            ...
            $data['domains'] = $this->qualification_domain_map_model->get_domain();
            $data['qualifications'] = $this->qualification_domain_map_model->get_qualification();
            $data['qualification_domain_map'] = $this->qualification_domain_map_model->searchQualificationDomainMap($qualification, $domain);

            $this->load->view($this->config->item('theme').'master/qualification_domain_map_view', $data);
        }
        else
        {
            redirect('admin/master/qualification_domain_map');
        }
    }
