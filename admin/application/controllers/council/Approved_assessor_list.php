...
class Approved_assessor_list extends NIC_Controller 
{
	function __construct()
	{
		parent::__construct();
        parent::check_privilege(44);
        $this->load->model('council/approved_assessor_list_model');
    ...
    public function index($offset = 0){
        ...
        $this->load->library('pagination');
            ...
            $config['base_url']         = 'council/approved_assessor_list/index/';
            $config['total_rows']       = $this->approved_assessor_list_model->assessor_count()[0]['count'];	
            $config['per_page']         = 25;
            $config['num_links']        = 2;
            ...
            $data['assessors']  	= $this->approved_assessor_list_model->get_assessor($config['per_page'],$offset);
            $data['page_links']     = $this->pagination->create_links();



		// 	$assessor_details=$this->approved_assessor_list_model->get_assessor_submission_time();
		// 	$cnt=0;
		// 	foreach($assessor_details as $assessor)
		// 	{
		// 		$submission_time=$assessor['assessor_table_time'];
		// echo $update_quesry="Update council_assessor_registration_application_nubmer set final_submission_time="."'".$submission_time."'"." where assessor_registration_details_fk=".$assessor['assessor_registration_details_pk'].";"."<br>";
		// 		$cnt++;
		// 	}
		// 	echo $cnt." no of Assessor code updated successfully";
           // $data['offset'] = $offset;
        } else {
			...
			if ($this->form_validation->run() == TRUE){
					$data['assessors'] 		= $this->approved_assessor_list_model->get_assessor_search($this->input->post('pan_no'));
			} 
			else {
        ...
        $this->load->view($this->config->item('theme').'council/approved_assessor_list_view',$data);
    }
    ...
    public function excel_download()
	{
		...
		$fileName = 'Approved_Assessor_'.date('Yms').'.xls';
		...
		$excel_data  	= $this->approved_assessor_list_model->get_assessor();
