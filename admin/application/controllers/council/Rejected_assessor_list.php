...
class Rejected_assessor_list extends NIC_Controller 
{
	function __construct()
	{
		parent::__construct();
        parent::check_privilege(45);
        $this->load->model('council/rejected_assessor_list_model');
    ...
    public function index($offset = 0){
        ...
        $this->load->library('pagination');
            ...
            $config['base_url']         = 'council/rejected_assessor_list/index/';
            $config['total_rows']       = $this->rejected_assessor_list_model->assessor_count()[0]['count'];	
            $config['per_page']         = 25;
            $config['num_links']        = 2;
            ...
            $data['assessors']  	= $this->rejected_assessor_list_model->get_assessor($config['per_page'],$offset);
           ...
           // $data['offset'] = $offset;
			...
			if ($this->form_validation->run() == TRUE){
					$data['assessors'] 		= $this->rejected_assessor_list_model->get_assessor_search($this->input->post('pan_no'));
			} 
			else {
        ...
        $this->load->view($this->config->item('theme').'council/rejected_assessor_list_view',$data);
    }
    ...
    public function excel_download()
	{
		...
		$fileName = 'Rejected_Assessor_'.date('Yms').'.xls';
		...
		$objPHPExcel->getProperties()->setCreator("Council")
									 ->setTitle("Rejected Assessor List")
									 ->setSubject("Rejected Assessor List")
									 ->setDescription("Rejected Assessor List")
									 ->setKeywords("Council")
									 ->setCategory("Council");
		...
		$excel_data  	= $this->rejected_assessor_list_model->get_assessor();
