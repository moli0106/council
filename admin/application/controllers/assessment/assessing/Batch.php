    ...
    public function index($offset = 0)
    {
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            if (!empty($batch_code) || !empty($sector_code) || !empty($course_code) || !empty($proposed_assessment_date) || !empty($assessment_status) || !empty($assessment_scheme)) {
                ...
                $data['batch_list']  = $this->assessing_batch_model->searchTraineeBatch($searchArray);
            } else {
            ...
            $config['base_url']         = 'assessment/assessing/batch/index/';
            $data["total_rows"]         = $config['total_rows'] = $this->assessing_batch_model->getTraineeBatchCount()[0]['count'];
            $config['per_page']         = 50;
            $config['num_links']        = 9;
            ...
            $data['batch_list']  = $this->assessing_batch_model->getAllTraineeBatch($config['per_page'], $offset);

            // echo "<pre>";print_r($data['batch_list']);exit;

            $data['excel_export'] = base_url('admin/assessment/assessing/batch/downloadAssessmentInExcel?sc=&cc=&as=&pad=&ast=');
        }

        // parent::pre($data);
        $this->load->view($this->config->item('theme') . 'assessment/assessing/batch_list_view', $data);
    }
        ...
        if ($this->input->server("REQUEST_METHOD") == "POST") {
            ...
            if($batch_id !=''){
                ...
                if($batch_delete){
                    ...
                    $delete_response_data = $this->assessing_batch_model->updateJsonData($batch_details['user_batch_id'], array('active_status' => 0,'delete_status'=>1));
